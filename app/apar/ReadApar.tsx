...
export default function ReadApar() {
  ...
  const handleDelete = (id_apar: string) => {
    Alert.alert('Hapus APAR','Yakin?',[
      {text:'Batal',style:'cancel'},
      {text:'Hapus',style:'destructive',onPress:async()=>{
        try {
          const res=await safeFetchOffline(`${baseUrl}/api/apar/${id_apar}`,{method:'DELETE'});
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <FlatList<APAR>
        data={displayList}
        keyExtractor={(item,idx)=>(item.id_apar.trim()||idx.toString())}
        renderItem={({item})=>(
          <View style={styles.card}>
            ...
            <View style={styles.actions}>
              <Pressable style={[styles.btn,styles.btnEdit]}
                onPress={()=>router.push({pathname:'/apar/EditApar',params:{id_apar:item.id_apar}})}>
...
const styles = StyleSheet.create({
  ...
  stats:{padding:16,backgroundColor:'#fff',borderBottomWidth:1,borderColor:'#ddd'},
